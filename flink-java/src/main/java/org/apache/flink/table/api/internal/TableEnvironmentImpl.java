...
public class TableEnvironmentImpl implements TableEnvironmentInternal {
    ...
    @Override
    public TableResult executeSql(String jobName, String statement) {
        List<Operation> operations = getParser().parse(statement);
        ...
        return executeInternal(jobName, operations.get(0));
    }
    ...
    private TableResult executeQueryOperation(QueryOperation operation) {
        return executeQueryOperation(null, operation);
    }

    private TableResult executeQueryOperation(String jobName, QueryOperation operation) {
        final UnresolvedIdentifier unresolvedIdentifier =
                UnresolvedIdentifier.of(
                        "Unregistered_Collect_Sink_" + CollectModifyOperation.getUniqueId() +
                                (Strings.isNullOrEmpty(jobName) ? "" : jobName));
        ...
        jobName = getJobName(Strings.isNullOrEmpty(jobName) ? "collect" : jobName);
        Pipeline pipeline = execEnv.createPipeline(transformations, tableConfig, jobName);
        try {
            JobClient jobClient = execEnv.executeAsync(pipeline);
    ...
    @Override
    public TableResult executeInternal(String jobName, Operation operation) {
        if (operation instanceof ModifyOperation) {
            return executeInternal(jobName, Collections.singletonList((ModifyOperation) operation));
        } else if (operation instanceof QueryOperation) {
            return executeQueryOperation(jobName, (QueryOperation) operation);
        } else {
