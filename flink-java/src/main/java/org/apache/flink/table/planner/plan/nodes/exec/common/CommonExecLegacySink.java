    ...
    protected Transformation<Object> translateToPlanInternal(PlannerBase planner) {
        if (tableSink instanceof StreamTableSink) {
            ...
            if (tableSink instanceof RetractStreamTableSink) {
                ...
                final TableSchema schema = upsertSink.getTableSchema();
                final boolean isAppendOnlyTable = !needRetraction;
                upsertSink.setIsAppendOnly(isAppendOnlyTable);
                String[] primaryKeys = schema.getPrimaryKey().isPresent() ?
                        schema.getPrimaryKey().get().getColumns().stream().toArray(String[]::new): null;
                primaryKeys = upsertKeys != null ? upsertKeys : primaryKeys;
                if (primaryKeys != null ) {
                    upsertSink.setKeyFields(primaryKeys);
                } else {
