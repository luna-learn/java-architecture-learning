...
public class DB2Dialect extends AbstractDialect {
    ...
    public Optional<String> getUpsertStatement(
        ...
        String values = Arrays.stream(fieldNames)
                .map(e -> ":" + e + " AS " + this.quoteIdentifier((e)))
                .collect(Collectors.joining(", "));
        String unique = Arrays.stream(uniqueKeyFields)
                .map(e -> "A." + this.quoteIdentifier((e)) + "=" + "B." + this.quoteIdentifier((e)))
                .collect(Collectors.joining(" AND "));
        String merge = "MERGE INTO " + tableName + " A"
                + " USING (SELECT " + values + " FROM SYSIBM.SYSDUMMY1) B"
                + " ON " + unique;

        String updateCause = Arrays.stream(fieldNames)
                .filter(e -> !Arrays.asList(uniqueKeyFields).contains(e))
                .map(e -> "A." + this.quoteIdentifier((e)) + "=" + "B." + this.quoteIdentifier((e)))
                .collect(Collectors.joining(", "));

        String update = " WHEN MATCHED THEN UPDATE SET " + updateCause;

        String insertColumns = Arrays.stream(fieldNames)
                .map(this::quoteIdentifier)
                .collect(Collectors.joining(", "));
        String insertCause = Arrays.stream(fieldNames)
                .map(e -> "B." + this.quoteIdentifier(e))
                .collect(Collectors.joining(", "));

        String insert = " WHEN NOT MATCHED THEN INSERT ("
                + insertColumns+ ") VALUES ("
                + insertCause + ")";

        Optional<String> statement = Optional.of(merge + update + insert);
